...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      >
        {/* Top Header */}
        <div className="flex-shrink-0 h-16 px-3.5 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
          {!isCollapsed ? (
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full shrink-0 flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all relative group cursor-pointer ${
                  isActive
        ...
        <div className="flex-shrink-0 border-t border-slate-100 dark:border-slate-800 p-3">
          {currentUser && !isCollapsed && (
            <div className="flex items-center gap-3 px-1 py-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
              <img
                src={currentUser?.avatar || defaultUserLogo}
                alt={currentUser?.fullName || 'User'}
                className="w-8 h-8 rounded-lg object-cover ring-2 ring-slate-300 dark:ring-slate-700 flex-shrink-0"
              />
              <div className="flex-1 min-w-0 text-left">
                <div className="text-xs font-bold text-slate-900 dark:text-white truncate">
                  {currentUser?.fullName}
                </div>
                <div className="text-[10px] font-medium text-slate-500 dark:text-slate-400 truncate uppercase tracking-wider">
                  {currentUser?.role}
                </div>
              </div>
            </div>
